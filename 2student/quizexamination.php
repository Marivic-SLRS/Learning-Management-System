...
function getscore($selectedAnswers, $conn){ 
  ...
  $sql = "SELECT Answer , Pointpernum FROM activity WHERE Tittle = '$exam_title' AND Classs = '$class' AND Subject = '$subject'";
  ...
  if ($result) {
   ...
   for ($i = 0; $i < count($right); $i++) {
    ...
    $finalscore = $sco . " " . $exam_title . " - " . $subject. " || ";
            ...
            $insertion = "UPDATE student SET quizscores = CONCAT(IFNULL(quizscores, ''), ?) WHERE full_name = ?";
...
$sql = "SELECT * FROM activity WHERE Recordid = '$recordid'";
$result = $conn->query($sql);
