          ...
          <!-- Content Header (Page header) -->
            <div class="content-header" style="background-color:darkgray; margin: 0;border-radius: 10px;">
              <div class="container-fluid">
                <div class="row mb-2">
                  <div class="col-sm-6" >
                    <h4 class="m-0" style="font-family: georgia; font-size: 14px;">Students Record -></h4>
                  </div>
                </div>
              </div>
            </div>
...
<style>
    ...
    th {
      background-color:rgb(3, 62, 62); /* Blue background color */
...
<style>
  ...
  .print-button {
    ...
    margin: 20px auto;
    padding: 10px 20px;
    background: linear-gradient(to bottom, rgba(3, 62, 62, 1), rgba(110, 95, 62, 1));
    color: white;
    border: none;
    border-radius: 5px;
    cursor: pointer;
...
<button class="print-button" onclick="printTables()">
     <i class="fas fa-duotone fa-print"></i>
     Print</button><br><br><br><br>
</head>
<body>
    ...
    <div class="card">
        <h6 align="center">Exam Scores</h6>
        <table class = "table-bordered table-striped" id="exam">
            <thead>
                <tr>
                    <th>Exam Scores</th>
                    <th>Title and Subject Code</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($examscores as $score) :
                    // Extract the first four integers
                    $matches = [];
                    preg_match('/\d{4}/', $score, $matches);
                    $exam_score = isset($matches[0]) ? $matches[0] : "";
                    
                    // Extract the subject code (assuming it's everything after the first four integers)
                    $subject_code = trim(str_replace($exam_score, '', $score));
                ?>
                <tr>
                    <td><?php echo htmlspecialchars($exam_score); ?></td>
...
// Fetch the student's exam scores
$sql = "SELECT quizscores FROM student WHERE id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $student_id);
$stmt->execute();
$result = $stmt->get_result();
...
if ($result->num_rows == 1) {
    $row = $result->fetch_assoc();
    $quizscores = array_map('trim', explode('||', $row['quizscores']));
} else {
    $quizscores = [];
}?>
    
        <h6 align="center">Quiz Scores</h6>
        <table class = "table-bordered table-striped" id="quiz">
            <thead>
                <tr>
                    <th>Quiz Scores</th>
                    <th>Title and Subject Code</th>
            ...
            <tbody>
                <?php foreach ($quizscores as $score) :
                    // Extract the first four integers
                    $matchess = [];
                    preg_match('/\d{4}/', $score, $matchess);
                    $quiz_score = isset($matchess[0]) ? $matchess[0] : "";
                    
                    // Extract the subject code (assuming it's everything after the first four integers)
                    $subject_code = trim(str_replace($quiz_score, '', $score));
                ?>
                <tr>
                    <td><?php echo htmlspecialchars($quiz_score); ?></td>
                    <td><?php echo htmlspecialchars($subject_code); ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
