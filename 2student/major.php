                            ...
                            <div style="display: inline-block; text-align: center; overflow: hidden; border-radius: 50%; width: 65px; height: 65px; border: 2px solid black; margin-bottom: 10px;">
                                ...
                                if ($result->num_rows > 0) {
                                    ...
                                    if (!empty($pictureFilename)) {
                                        echo '<a href="hhttp://localhost/Learning%20Management%20System/2student/profilepics.php"><img src="http://localhost/Learning%20Management%20System/2student/profiles/' . $pictureFilename . '" alt="PROFILE PICTURE" class="img-fluid" style="max-width: 100%; height: auto; display: block; margin: 0 auto;"></a>';
                                    } else {
...
function isExamTaken($conn, $fullname, $check_value, $sub_value) {
    $sql_taken = "SELECT * FROM student WHERE full_name = '$fullname' AND examscores LIKE  '%$check_value%' AND examscores LIKE '%$sub_value%'";
...
<table id="examTable" class ="table-bordered container-fluid table-striped">
    ...
    <tbody>
        <?php
        $sql = "SELECT * FROM major WHERE classs = '$class'";
        $result_exams = $conn->query($sql);
        while ($row_exam = $result_exams->fetch_assoc()) {
            // Check if the exam has been taken by the current user
            $exam_id = $row_exam['recordid'];
            $check_value = $row_exam['tittle']; // Change this to the appropriate value you want to check
            ...
            <tr class="classRow <?= $taken_class ?>">
                ...
                <td>
                    ...
                    <?php else: ?>
                        <a href="http://localhost/Learning%20Management%20System/2student/majorexamination.php?exam_id=<?= $row_exam['recordid']; ?>&tittle=<?= $row_exam['tittle']; ?>&subject=<?= $row_exam['subject']; ?>&teachername=<?= $row_exam['teachername']; ?>&point=<?= $row_exam['pointpernum']; ?>&type=<?= $row_exam['type']; ?>"  class="btn btn-gradient take-exam-btn" >Take Exam</a>
                    <?php endif; ?>
                </td>
            </tr>
        <?php } ?>
