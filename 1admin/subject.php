...
if (!isset($_SESSION["username"]) || empty($_SESSION["username"])) {
    ...
    function insertData($subject_code, $department, $description) {
     
      $conn = getDB();
      $sql = "INSERT INTO `subjects` (`subject_code`, `description`, `department_name`)
              VALUES ('$subject_code', '$department', '$description')";
      $conn->query($sql);
...
function updateData($id, $subject_code, $description, $department) {
    $conn = getDB();
    $sql = "UPDATE `subjects` SET `subject_code`= '$subject_code', `department_name`= '$department', `description`= '$description' WHERE `id`= '$id'";
    $conn->query($sql);
      header("Location: subject.php"); 
       exit();
}
...
if(isset($_POST['submit'])){
  // Call the insertData function with form data
  insertData($_POST['SubjectCode'], $_POST['department'], $_POST['Description']);
}
...
if(isset($_POST['Update'])){
    updateData($_POST['idDept'], $_POST['SubjectCode'], $_POST['Department'], $_POST['Description']);
header("Location: subject.php"); 
         exit();
...
<div class="content-wrapper" style="min-height: 567.854px;">
      ...
      <!-- Content Header (Page header) -->
        <div class="content-header" style="background-color:darkgray;border-radius: 10px;">
          <div class="container-fluid">
            <div class="row mb-2">
              <div class="col-sm-6" >
                <h4 class="m-0" style="font-family: georgia; font-size: 14px;">Subjects -></h4>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="container mt-5">
        <div class="card p-3"  style="background: linear-gradient(to right, lightgray, white);">
          <legend class="border-bottom border-5">Add Subject code</legend>
          <div class="card-body">
            <form method="post">
            <div class="mb-3">
              <label for="exampleFormControlInput1" class="form-label">Subject Code</label>
              <input type="text" class="form-control" id="exampleFormControlInput1" placeholder="Enter Subject Code" name="SubjectCode" required>
            </div>
             <div class="mb-3">
      ...
      <div class="container mt-5" id="updateForm" style="display: none;">
        <div class="card p-3"  style="background: linear-gradient(to right, lightgray, white);">
          <legend class="border-bottom border-5">Update Department</legend>
          <div class="card-body">
            <form method="post">
...
<div class="container mt-5">
    ...
    <thead style="background: linear-gradient(to bottom, darkred, black);" white; border-top-left-radius: 10px; border-top-right-radius: 10px;>
                    <tr>
                        ...
                        <th style="border-top-left-radius: 10px; border-top-right-radius: 10px; color: white">Subject Code</th>
                        <th style="border-top-left-radius: 10px; border-top-right-radius: 10px; color: white">Department</th>
                        <th style="border-top-left-radius: 10px; border-top-right-radius: 10px; color: white">Description</th>
   ...
   <?php
            ...
            if ($result->num_rows > 0) {
             ...
             <tr>
              ...
              <td>
                <a  href="#" onclick="updateDep(<?php echo $row['id'];?>, '<?php echo addslashes($row['subject_code']);?>', '<?php echo addslashes($row['department_name']);?>', '<?php echo $row['description'];?>')" class="btn btn-warning">Update</a>
    ...
    <script>
    function updateDep(id, subject_code, department_name ,description ) {
      console.log('working');
      document.getElementById('idDept').value = id;
      document.getElementById('SubjectCode').value = subject_code;
      document.getElementById('Description').value = department_name;
      document.getElementById('Department').value = description;
