...
function insertData($class_name, $department) {
    $conn = getDB();
     $sql = "INSERT INTO `class`(`class_name`, `department`) VALUES ('$class_name','$department')";
     $conn->query($sql);
...
function updateData($id, $Class_name, $Department) {
  $conn = getDB();
  $sql = "UPDATE `class` SET `class_name`= '$Class_name' , `department`='$Department' WHERE `id`= '$id'";
  $conn->query($sql);
   header("Location: class.php"); 
       exit();
}
...
if(isset($_POST['submit'])){
  insertData( $_POST['class_name'],$_POST['department']);
}
...
if(isset($_POST['Update'])){
  updateData($_POST['idDept'], $_POST['Class_name'], $_POST['Department']);
}
...
<div class="content-wrapper" style="min-height: 567.854px;">
    ...
    <div class="content-header" style="background-color: darkgray;border-radius: 10px;">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h4 class="m-0" style="font-family: georgia; font-size: 14px;">Class -></h4>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="container mt-5" > 
        <div class="card p-3" style="background: linear-gradient(to right, lightgray, white);">
          <legend class="border-bottom border-5" >Add Class</legend>
          <div class="card-body">
            <form method="post">
            <div class="mb-3">
              <label for="exampleFormControlInput1" class="form-label">Enter Class</label>
              <input type="text" class="form-control" id="class_name" placeholder="Enter Class Name" name="class_name" required>
            </div>
 <!-- department -->
 <div class="mb-3">
      ...
      <div class="container mt-5" id="updateForm" style="display: none;">
        <div class="card p-3" style="background: linear-gradient(to right, lightgray, white);">
          ...
          <div class="card-body">
            ...
            <div class="mb-3">
              <label for="exampleFormControlInput1" class="form-label">Department</label>
              <input type="text" class="form-control" id="Class_name" placeholder="Enter Depertment" name="Class_name" required>
            </div>
 <!-- department -->
 <div class="mb-3">
                        <label for="exampleFormControlInput1" class="form-label">Department</label>
...
<style>
    ...
    <div class="table-responsive table-responsive-lg">
        ...
        <thead style="background: linear-gradient(to bottom, darkred, black);" white; border-top-left-radius: 10px; border-top-right-radius: 10px;>
             <tr>
                        ...
                        <th style="border-top-left-radius: 10px; border-top-right-radius: 10px; color: white">Name</th>
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                 ...
                 <tr>
                  ...
                  <td>
                    <a  href="#" onclick="updateDep('<?php echo $row['id'];?>', '<?php echo $row['class_name'];?>','<?php echo $row['department'];?>')" class="btn btn-warning">Update</a>
    ...
    function updateDep(id, department, description) {
      ...
      document.getElementById('Class_name').value = department;
