...
function updateData($id, $department, $class, $subject) {
  $conn = getDB();
  $sql = "UPDATE `facultys` SET `department`='$department', `class`='$class', `subject`='$subject' WHERE `id`='$id'";
  $conn->query($sql);
...
if(isset($_POST['Assign'])){
  ...
  // Assuming $_POST['Classs'] and $_POST['Subject'] are arrays
  $class = rtrim(implode(',', $_POST['Classs']), ',');
  $subject = rtrim(implode(',', $_POST['Subject']), ',');
  updateData($id, $department, $class, $subject);
}
if (isset($_GET['delete'])) {
  $conn = getDB();
  $id = $_GET['delete'];
  
  // Set department to an empty string or a specific default value instead of NULL
  $sql = "UPDATE facultys SET department='', class='', subject='' WHERE id='$id'";
...
<!-- Contains page content -->
   ...
   <!-- Update Teacher Form -->
     ...
     <div class="card p-3" style="background: linear-gradient(to right, lightgray, white);">
          <legend class="border-bottom border-5">Update faculty Information</legend>
          <div class="card-body">
            <form method="post">
            <input type="hidden" class="form-control" placeholder="Teacher ID Number" name="id" id="id" readonly>
                     
            <div class="mb-3">
              <label for="exampleFormControlInput1" class="form-label">Full Name</label>
              <input type="text" class="form-control" id="fullname" placeholder="Enter Last Name" name="fullname" required readonly>
            </div>
            <div class="mb-3">
              <label for="exampleFormControlInput1" class="form-label">Qualification</label>
              <input type="text" class="form-control" id="qualification" placeholder="Enter Qualification" name="qualification" required readonly>
            </div>
           
            <legend class="border-bottom border-5">Assign Information</legend> <br>
                        <!-- department-->
                        <div class="mb-3">
                          ...
                          <div class="mb-3">
                            ...
                            <select name="Classs[]" id="class" multiple require>
                              S
    ...
    onChange: function(values) {
        console.log(values)
            ...
            <input type="submit" name="Assign" value="Assign" class="btn btn-primary" onclick="return checkContactNumberLength()">
            </form>
          </div>
        </div>
      </div>
...
<br><br>
<h3 style = "text-align: center;"> Teachers Assignments </h3>
...
<div class="container mt-5">
    ...
    <thead style="background: linear-gradient(to bottom, darkred, black);" white; border-top-left-radius: 10px; border-top-right-radius: 10px;>
                    <tr>
                        ...
                        <th style="border-top-left-radius: 10px; border-top-right-radius: 10px; color: white">Subject</th>
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="#" onclick="displayUpdateForm('<?php echo $row['id'];?>','<?php echo $row['fullname'];?>', 
                            '<?php echo $row['qualification'];?>','<?php echo $row['department'];?>' , '<?php echo $row['class'];?>',
                            '<?php echo $row['subject'];?>')" class="btn btn-warning">Assign</a>
                           <a href="?delete=<?php echo $row['id'];?>" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete?')">Delete</a>
   ...
   function displayUpdateForm(id, fullname, qualification, ) {
