...
function insertData($employee_id, $fullname, $address, $contact_no, $gender, $qualification, $emergency_person, $emergency_num) {
    $password = "instructor111"; 
    $conn = getDB();

    if (!$conn) {
        return "<p>Error: Connection failed: " . mysqli_connect_error() . "</p>";
    }

    // Prepare an SQL statement
    $sql = "INSERT INTO `facultys` (`employee_id`, `password`, `fullname`, `address`, `contact_no`, `gender`, `qualification`, `emergency_person`, `emergency_num`, `code`, `department`, `subject`, `class`, `picture`) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, '', '', '', '')";

    // Bind parameters to the prepared statement
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("issssssss", $employee_id, $password, $fullname, $address, $contact_no, $gender, $qualification, $emergency_person, $emergency_num);
    ...
    if ($stmt->execute()) {
        ...
        // Return a success message instead of echoing directly
        return "<p>Data inserted successfully.</p>";
    } else {
        // Return an error message on failure
        return "<p>Error: " . $stmt->error . "</p>";
    }

    // Close the connection (not necessary, as PHP will automatically close the connection when the script ends)
    // $conn->close();
}



function updateData($id, $employee_id, $fullname, $address, $contact_no, $gender, $qualification, $emergency_person, $emergency_num) {
  ...
  try {
      ...
      // Prepare an SQL statement for updating data
      $sql = "UPDATE `facultys` SET 
              `employee_id` = ?,
              `fullname` = ?,
              `address` = ?,
              `contact_no` = ?,
      ...
      // Bind parameters to the prepared statement
      $stmt = $conn->prepare($sql);
      $stmt->bind_param("isssssssi", $employee_id, $fullname, $address, $contact_no, $gender, $qualification, $emergency_person, $emergency_num, $id);
...
if(isset($_POST['update'])){
  // Assuming you have a function called updateData to handle updating data
  updateData($_POST['id'], $_POST['employee_id'], $_POST['full_name'], $_POST['address'], $_POST['contact_no'], $_POST['gender'], $_POST['qualification'], $_POST['emergency_person'], $_POST['emergency_num']);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  // Validate and sanitize input data (You should implement this according to your requirements)

  // Update teacher information in the database
  $conn = getDB(); // Assuming getDB() function retrieves your database connection
  ...
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("isssssssi", $employee_id, $full_name, $address, $contact_no, $gender, $qualification, $emergency_person, $emergency_num, $id);
...
if(isset($_GET['id'])) {
  $teacher_id = $_GET['id'];
  $conn = getDB();
  $sql = "SELECT * FROM facultys WHERE id =?";
  $stmt = $conn->prepare($sql);
...
function deleteData($id) {
  $conn = getDB();
  $sql = "DELETE FROM facultys WHERE id =?";
  $stmt = $conn->prepare($sql);
...
<!-- Contains page content -->
      ...
      <!-- Content Header (Page header) -->
        <div class="content-header" style="background-color:darkgray;border-radius: 10px;">
          <div class="container-fluid">
            <div class="row mb-2">
              <div class="col-sm-6" >
                <h4 class="m-0" style="font-family: georgia; font-size: 14px;">Manage Teacher Account -></h4>
              </div>
            </div>
          </div>
        </div>
    ...
    <br>
     <h4 style = "text-align: center;"> Teachers Information </h4>
        ...
        <div class="card p-3" style="background: linear-gradient(to right, lightgray, white);">
          <legend class="border-bottom border-5">Add Teacher</legend>
          <br>
          <div class="card-body">
            <form method="post">
                  
            <input type="hidden" class="form-control" placeholder="Employee ID Number" name="id" id="id" readonly>
           
           
            <div class="mb-3">
            
              <label for="exampleFormControlInput1" class="form-label">Employee ID</label>
              <input type="number" class="form-control" id="employee_id" placeholder="Enter Employee ID" name="employee_id" required min="0" step="1" oninput="this.value = this.value.slice(0, 50)">
            </div>
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="emergency_person" placeholder="Enter Full Name" name="emergency_person" required>
            </div>
            <div class="mb-3">
              <label for="exampleFormControlInput1" class="form-label">Emergency Contact no.</label>
              <input type="text" class="form-control" id="emergency_num" placeholder="Enter Contact no." name="emergency_num" pattern="[0-9]{11}" title="Please enter an 11-digit number" required>
            </div>
        
            
            <legend class="border-upper border-5">Default Password: instructor111</legend> <br>           
    ...
    <div class="card p-3"  style="background: linear-gradient(to right, lightgray, white);">
        ...
        <br>
                ...
                if(isset($_GET['id'])) {
                    $teacher_id = $_GET['id'];
                    $conn = getDB();
                    $sql = "SELECT * FROM facultys WHERE id = $teacher_id";
                    $result = $conn->query($sql);
                    if ($result->num_rows == 1) {
                        $row = $result->fetch_assoc();
                        ...
                        ?>
                         <div class="card-body">
                        <form method="post" action="teacher_acc.php">
                         <legend class="border-bottom border-5">Update Teacher</legend>
              ...
              <label for="exampleFormControlInput1" class="form-label">Employee ID</label>
              <input type="number" class="form-control" id="employee_id" placeholder="Enter Employee ID" name="employee_id" value="<?php echo $row['employee_id']; ?>" required min="0" step="1" oninput="this.value = this.value.slice(0, 50)">
            </div>
            
            
                        <div class="mb-3">
                            <label for="full_name" class="form-label">Full Name</label>
                            <input type="text" class="form-control" id="full_name" placeholder="Enter Full Name" name="full_name" value="<?php echo $row['fullname']; ?>" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="Contact_no" placeholder="Enter Contact no." name="contact_no" value="<?php echo $row['contact_no']; ?>"  pattern="[0-9]{11}" title="Please enter an 11-digit number format 09" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="qualification" name="qualification" required>
                                <option value="">Select qualification</option>
                                <option value="Licensed Teacher" <?php echo ($row['qualification'] == 'Licensed Teacher') ? 'selected' : ''; ?>>Licensed Teacher</option>
                                <option value="Bachelor Degree Holder" <?php echo ($row['qualification'] == "Bachelor Degree Holder") ? 'selected' : ''; ?>>Bachelor Degree Holder</option>
                                <option value="Master Degree Holder" <?php echo ($row['qualification'] == "MasterDegree Holder") ? 'selected' : ''; ?>>Master Degree Holder</option>
                                <option value="Doctoral Degree Holder" <?php echo ($row['qualification'] == "Doctoral Degree Holder") ? 'selected' : ''; ?>>Doctoral Degree Holder</option>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="emergency_person" placeholder="Enter Full Name" name="emergency_person" value="<?php echo $row['emergency_person']; ?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="emergency_num" class="form-label">Emergency Contact no.</label>
                            <input type="text" class="form-control" id="Emergency_num" placeholder="Enter Contact no." name="emergency_num" value="<?php echo $row['emergency_num']; ?>"  pattern="[0-9]{11}" title="Please enter an 11-digit number format 09" required>
                        </div>
                         <legend class="border-upper border-5">Default Password: instructor111</legend> <br>
...
<style>
   ...
   <!-- Table -->
    ...
    <thead style="background: linear-gradient(to bottom, darkred, black);" white; border-top-left-radius: 10px; border-top-right-radius: 10px;>
            ...
            <th style="border-top-left-radius: 10px; border-top-right-radius: 10px; color: white">Employee ID</th>
            ...
            <th style="border-top-left-radius: 10px; border-top-right-radius: 10px; color: white">Gender</th>
            <th style="border-top-left-radius: 10px; border-top-right-radius: 10px; color: white">Qualification</th>
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td><?php echo $row['emergency_person']; ?></td>
                        <td><?php echo $row['emergency_num']; ?></td>
                        <td>
                            <a href="teacher_acc.php?id=<?php echo $row['id'];?>" class="btn btn-primary">Edit</a>
...
function checkContactNumberLengthadd() {
              ...
              if (contact_no.length != 11 && emergency_num.length != 11) {
                  alert("Error: Please check your Contact Number length");
                  return false; 
              } else if (contact_no.length != 11 || emergency_num.length != 11) {
                  alert("Error: Please check your Contact Number length");
                  return false; 
              }
              return true; 
          ...
          function checkContactNumberLength() {
              var contact_no = document.getElementById('Contact_no').value;
              var contact_num = document.getElementById('Emergency_num').value;
