...
<div class="content-wrapper " id = "all" style="min-height: 567.854px; background-image: url(http://localhost/Learning%20Management%20System/images/logo.png);background-repeat: no-repeat;  
  background-size: 40%;background-position: center;  background-blend-mode: soft-light;
          ...
          <!-- end of header -->
                ...
                <?php 
                      ...
                      if ($conn) {
                          ...
                          $filesQuery = "SELECT COUNT(*) as filename FROM files WHERE teachername = '$fullname'"; 
                          $classQuery = "SELECT class FROM facultys WHERE fullname = '$fullname'"; 
                  
                          $quizQuery = "SELECT COUNT(*) as Tittle FROM activity WHERE teachername = '$fullname'"; 
                          $examQuery = "SELECT COUNT(*) as tittle FROM major    WHERE teachername = '$fullname'"; 
                          ...
                          if ($result) { 
                              ...
                              $class_list = explode(',', $result->fetch_assoc()['class']);
   ...
   <style>
    ...
    .card {
        background: linear-gradient(to bottom, rgba(21, 27, 84, .9), rgba(43, 84, 126, .9));
        border-radius: 10px;
        padding: 20px;
        color: white;
        display: flex;
...
<div class="dashboard-container">
    <?php
        $data = array(
            array("fa-file-pdf", "Total Uploaded Files", $filesnum),
            array("fa-users", "Total Classes", $class_count),
            array("fa-file", "Total Quizzes", $quiznum),
            array("fa-file-pen", "Total Major Exams", $majornum)
        );
    ...
    <div class="card">
        ...
        <div class="info">
            <div class="number"><?php echo isset($item[2]) ? $item[2] : 'N/A'; ?></div>
            <div class="label"><?php echo $item[1]; ?></div>
        </div>
    </div>
    <?php } ?>
</div>
