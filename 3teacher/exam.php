...
function fetchData() {
  global $fullname;
  $conn = getDB(); 
  $sql = "SELECT * FROM `major` WHERE teachername = '$fullname'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
  ...
  for ($i = 1; $i <= $numQuestions; $i++) {
      ...
      // Concatenatse question, choices, and answer
  ...
  // Insert values into the 'major' table
  $conn = getDB();
  $sql = "INSERT INTO major (tittle, classs, subject, teachername,  quesnum, pointpernum, questions, choice, answer, type) VALUES (?, ?, ?, ?, ?, ?, ?,?,?,?)";
  ...
  // Rest of your code for processing form data goes here
  // Ensure to validate and sanitize the input data as needed
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['isubmit'])) {
  ...
  $conn = getDB(); 
  if ($conn) {
      $sql = "INSERT INTO major  (tittle, classs, subject, teachername, quesnum, pointpernum, questions, answer, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)";
      $stmt = $conn->prepare($sql);
      if ($stmt) {
...
function deleteData($recordid) {
  $conn = getDB();
     $sql = "DELETE FROM `major` WHERE recordid = '$recordid'";
     $conn->query($sql);
...
function updateData($recordid, $title, $classs,$subject) {
  $conn = getDB();
  $sql = "UPDATE `major` SET `tittle`= '$title' , `classs`='$classs', `subject`='$subject' WHERE `recordid`= '$recordid'";
  $conn->query($sql);
...
<head>
    ...
    <!-- Theme style -->
    <link rel="stylesheet" href="hhttp://localhost/Learning%20Management%20System/css/custom.css">
              ...
              <br>
                <div style="text-align: center;">
                            ...
                            <div style="display: inline-block; text-align: center; overflow: hidden; border-radius: 50%; width: 65px; height: 65px; border: 2px solid black; margin-bottom: 10px;">
                              ...
                              <!-- Full name link -->
                              <div class="info">
                                  <p><a href="http://localhost/Learning%20Management%20System/http://localhost/Learning%20Management%20System/3teacher/account.php" class="d-block" style="margin:7px; font-family: georgia;">
          ...
          <!-- Content Header (Page header) -->
            <div class="content-header" style="background-color:darkgray; margin: 0;border-radius: 10px;">
              <div class="container-fluid">
                <div class="row mb-2">
                  <div class="col-sm-6" >
                    <h4 class="m-0" style="font-family: georgia; font-size: 14px;">Major Exam -></h4>
                  </div>
                </div>
              </div>
            </div>
            ...
            <nav class="navbar navbar-expand-lg bg-body-tertiary">
              <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                  <ul class="navbar-nav">
                    <li class="nav-item">
                      <a class="nav-link" href="#existing" data-target="ex">Existing Exam</a>
                    </li>
                    <li class="nav-item">
                      <a class="nav-link" href="#create" data-target="cre">Add Exam</a>
                    </li>
                  </ul>
                </div>
              </div>
                  ...
                  <div id="existing">
                      <div class="card p-3">
                          <div class="card-body">
                              ...
                              <!-- Table -->
                              <h3 class="text-center">Major Exam</h3>
    ...
    <div class="table-responsive table-responsive-lg">
                  ...
                  <!-- create -->
                    <div id="create" style="display: none;">
                          <br><br>
                          <div class="card p-3">
                              <legend class="border-bottom border-5 text-center">Exam Details</legend>
                              <div class="card-body">
                                  <form method="post" action="exam.php" id="examForm">
                                      <div class="mb-3">
                                          <label for="title" class="form-label">Title</label>
                                          <input type="text" class="form-control" id="title" placeholder="Enter Exam Title" name="title" required>
                                      </div>
                                      <div class="mb-3">
                                          <label for="classs" class="form-label">Class</label>
                    ...
                    <script>
                        document.getElementById('nextBtn').addEventListener('click', function() {
                            ...
                            for (var i = 1; i <= numq; i++) {
                                ...
                                questionFields.innerHTML += '<b>Quesiton number&nbsp;' + i + '&nbsp;:</b><br />' +
                                   // question 
                                   '<div class="form-group">' +
                            ...
                            if (typetest === 'multiple') {
                                    questionFields.innerHTML += '<div class="form-group">' +
                                        '<div class="col-md-12">' +
                                        '<input type="submit" name="submit" value="Submit" style = "background-color: blue;" class="btn btn-primary" onclick="return confirm(\'Please double check before submitting, this action cannot be undone.\');">' +
                                        '</div>' +
                                        '</div>';
                                } else if (typetest === 'identification') {
                                    questionFields.innerHTML += '<div class="form-group">' +
                            ...
                            document.getElementById('questions').style.display = 'block'; // Show questions section
                        });
                    </script>
