...
include 'dbconnections/dbadminlogin.php';
  if (isset($_SESSION["reset_studentId"])) {
	   $reset_studentId = $_SESSION["reset_studentId"];
		...
		if (!empty($reset_studentId)) {
			$sql = "SELECT code FROM student WHERE Student_id='$reset_studentId'";
		   ...
		   if (isset($_GET["code"])) {
                            $code = $_GET["code"];                          
                                if ($code === $dbcode) {
                                    $_SESSION["reset_studentId"] = $reset_studentId;
									 $_SESSION["studentId"] = $row["studentId"]; 
                                    $redirect_url = "Snew-password.php?studentId=" . urlencode($reset_studentId);
									 header("Location: $redirect_url"); // Redirect to Anew-password.php                      
                                    exit();
                                } else {
                                    header("Location: Sdenied.php"); // Redirect to Adenied.php if the codes don't match
                                    exit();
                                }                         
                            }
  }}}
